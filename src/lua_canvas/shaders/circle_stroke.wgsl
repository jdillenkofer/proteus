// Circle stroke shader using SDF
...
// Circle stroke SDF (ring)
fn circle_stroke_sdf(p: vec2<f32>, center: vec2<f32>, radius: f32, stroke_width: f32) -> f32 {
    let dist_from_center = length(p - center);
    return abs(dist_from_center - radius) - stroke_width * 0.5;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let dist = circle_stroke_sdf(pixel_pos, center, radius, stroke_width);
